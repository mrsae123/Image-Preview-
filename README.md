...
- "Reset floating button position" moved out of Settings and into a one-tap action in the chat's right sidebar — a settings switch had no way to turn itself back off, so leaving it on kept silently re-resetting the button on every chat open. The new action resets it once, snaps it back on screen immediately if it's visible, and confirms with a small pulse and a toast.
- Quick Lookup history (Translate / Meaning / Web Search) is now saved per chat instead of one shared device-wide list, so history from different conversations no longer mixes together.
